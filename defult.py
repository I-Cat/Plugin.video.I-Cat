...
addon=xbmcaddon.Addon('plugin.video.I-Cat')
...
def getSources(fanart):
    html=getRequest('https://www.youtube.com/')
...
def getCats(c_url):
    cat_url = 'https://www.youtube.com/'%(c_url)
...
def getShow(sid):
      html=getRequest('https://www.youtube.com/%s' % (sid))
...
elseif mode=='SR':xbmcplugin.setResolvedUrl(int(sys.argv[1]),True,xbmcgui.ListItem(path=p('url')))
elseif mode=='PP':play_playlist(p('name'),p('playlist'))
elseif mode=='GC':getCats(p('url'))
elseif mode=='GS':getShow(p('url'))
